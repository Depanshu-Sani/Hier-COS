...
CUSTOM_MODELS = ["custom_resnet50", "haframe_resnet50", "wide_resnet", "haframe_wide_resnet", "vit", "haframe_vit"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature_space", type=str, default=None)
    parser.add_argument("--orthonormal_basis_vectors", default=None)
    parser.add_argument("--alpha", type=float, default=1)
    parser.add_argument("--viz_gradcam", type=int, default=None)
    ...
    if opts.start == "training":
        ...
        elif opts.data == "wildlife-species":
            project = "wildlife-species"
        else:
            raise ValueError(f"opts.data {opts.data} unknown")

        entity = "hierarchy-aware-feature-space"
        if opts.loss == "cross-entropy":
            if opts.feature_space == "hier-cos":
                run_name = f"hier-cos ({opts.alpha},{opts.haf_gamma})"
            else:
                run_name = f"cross-entropy"
        else:
            run_name = f"{opts.loss}"

        if opts.tag:  # additional tag
            project = f"{project}-{opts.tag}"

        if opts.seed is not None:
            run_name = f"[{opts.seed}] {run_name}"
