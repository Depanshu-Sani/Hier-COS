...
def main(test_opts):
    ...
    if opts.start == "testing":
        ...
        # test_dataset, val_dataset, test_loader, val_loader = data_loader.train_data_loader(opts)
    ...
    if os.path.exists(f'{opts.out_folder}/test_dist_{opts.feature_space}.npy'):
        if os.path.exists(f'{opts.out_folder}/LCA_similarity.npy'):
            os.remove(f'{opts.out_folder}/LCA_similarity.npy')
        os.remove(f'{opts.out_folder}/test_dist_{opts.feature_space}.npy')
        os.remove(f'{opts.out_folder}/test_targets_{opts.feature_space}.npy')
    ...
    elif "vit" in opts.arch:
        print("Using pretrained ViT. Inference only!!!")
    else:
    ...
    summary, _ = run(test_loader, model, loss_function, distances, soft_labels,
                     classes, opts, 0, 0, is_inference=True, corrector=corrector, hiercos_params=hiercos_params)
