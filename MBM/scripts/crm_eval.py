...
def logit_extraction(data_loader, model, opts, verbose=True, distances=None, classes=None):
    ...
    with torch.no_grad():
        for i, (input_var, target) in enumerate(data_loader):
            ...
            if opts.feature_space == "hier-cos":
                loss, logit_var = loss_function(model(input_var, target_var), target_var)
            elif opts.arch in ["custom_resnet50", "wide_resnet"]:
    ...
    print("Level-wise AHD metrics")
    predictions = loss_function.predictions
    labels = loss_function.labels
    num_classes = len(classes)
    C = torch.tensor([[0 for i in range(num_classes)] for j in range(num_classes)]).cuda()
    top_k = [0, 5, 20]
    for i in range(num_classes):
        for j in range(num_classes):
            C[i][j] = distances[(classes[i], classes[j])]
    max_level = C.max()
    for level in range(max_level): # specify the levels at which AHD needs to be computed
        top_preds = torch.tensor(np.array(predictions[level])).topk(min(np.array(predictions[level]).shape[1], top_k[-1]))[1]
        level_gt = torch.tensor([torch.where(loss_function.hiercos_params["leaf_class_to_hierarchical_labels"][:, level] == torch.tensor(np.array(labels[level])).cuda()[i])[0].max().item() for i in range(len(predictions[level]))]).cuda()
        for k in range(top_preds.shape[1]):  # Handles the case where K_l < top_k (i.e., the number of nodes at level l is less than k)
            level_labels = torch.tensor([torch.where(loss_function.hiercos_params["leaf_class_to_hierarchical_labels"][:, level] == top_preds[:, k].cuda()[i])[0].max().item() for i in range(len(predictions[level]))]).cuda()
            if k == 0: # k = 0 signifies the top-1 prediction that gives us Mistake Severity
                wrongs = torch.where(level_gt != level_labels)[0]
                print(f"[{level}] MS:", C[level_labels[wrongs], level_gt[wrongs]].double().mean() - (max_level - level) + 1)
            level_wise_avg_hdist.append((C[level_labels, level_gt].double() - (max_level - level) + 1).clip(0, max_level).mean().item())
            if (k + 1) in top_k or (k + 1) == top_preds.shape[1]:
                print(f"[{level}] Avg Hier Dist @ {k + 1}:", np.mean(level_wise_avg_hdist))
...
def main(opts):
    ...
    opts, hiercos_params = get_hiercos_parameters(opts, classes, distances, only_fine_labels=False, fa=False, fd=False)
    opts.hiercos_params = hiercos_params
    ...
    opts.hierarchy = lca_to_hierarchy(hiercos_params["distance_matrix"].detach().cpu().numpy())
    ...
    logit, gt = logit_extraction(test_loader, model, opts, True, distances, classes)
