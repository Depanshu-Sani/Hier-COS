...
from util.arch import wideresnet, custom_wideresnet, custom_resnet, custom_vit, HAF_resnet
...
def load_pretrained_resnet50(opts):
    if opts.data in ["inaturalist19-224", "fgvc-aircraft", "wildlife-species"]:
...
def init_model_on_gpu(gpus_per_node, opts, distances=None):
    ...
    if opts.arch == "wide_resnet":
        ...
        if opts.loss == "cross-entropy" and opts.feature_space != "hier-cos":
            ...
            haf_cls_weights = opts.orthonormal_basis_vectors
        ...
        if opts.data == "cifar-100":
            ...
            model = custom_vit.CustomViTForImageClassification.from_pretrained('vesteinn/vit-mae-inat21')
            feature_dim = 1024
            standard_vit = False
        else:
        ...
        if opts.loss == "cross-entropy":
            model = custom_vit.ViT(model, num_classes=opts.num_classes, feature_size=feature_dim, standard_vit=standard_vit)
        ...
        if opts.data == "cifar-100":
            ...
            standard_vit = True
        if opts.loss == "cross-entropy" and opts.feature_space != "hier-cos":
            model = custom_vit.HAFrameViT(model, num_classes=opts.num_classes, haf_cls_weights=None)
        elif opts.loss == "cross-entropy" and opts.feature_space == "hier-cos":
            model = custom_vit.HAFrameViTNet(model, num_classes=opts.num_classes, feature_size=feature_dim, standard_vit=standard_vit)
            haf_cls_weights = opts.orthonormal_basis_vectors
            model = custom_vit.HAFrameViT(model,
                                          num_classes=opts.num_classes,
                                          haf_cls_weights=haf_cls_weights)
        elif opts.loss == "mixed-ce-gscsl" or opts.loss == "fixed-ce":
            assert opts.haf_gamma > 0
